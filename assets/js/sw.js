...
self.addEventListener("install", (e) => {
  e.waitUntil(
    caches.open(CURRENT_CACHES.static).then(function (e) {
      return e.addAll([
        "../../vendor.bundle.js",
        "../../app.bundle.js",
        "../../app.css",
  ...
  self.addEventListener("fetch", (e) => {
    e.respondWith(
      caches.open(CURRENT_CACHES.assets).then((t) =>
        t
          .match(e.request)
          .then(
            (s) =>
              s ||
              fetch(e.request.clone()).then(
                (s) => (
                  s.status < 400 &&
                    s.headers.has("content-type") &&
                    (s.headers.get("content-type").match(/^font\//i) ||
                      s.headers.get("content-type").match(/^image\//i)) &&
                    t.put(e.request, s.clone()),
                  s
                )
              )
          )
